      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100">
            {['ID', 'Status', 'Created', 'Completed', 'COT (°C)', 'Feed (kg/h)'].map(h => (
        ...
        <tbody>
          {data.map(t => (
            <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
              <td className="py-3 pr-6 font-mono text-gray-500">#{t.id}</td>
              <td className="py-3 pr-6"><StatusBadge status={t.status} /></td>
              <td className="py-3 pr-6 text-gray-500">{fmt(t.created_at)}</td>
              <td className="py-3 pr-6 text-gray-500">{fmt(t.completed_at)}</td>
              <td className="py-3 pr-6 tabular-nums">{t.cot_input?.toFixed(1) ?? '—'}</td>
              <td className="py-3 pr-6 tabular-nums">{t.flow_input?.toFixed(0) ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        ...
        {selected.length > 0 && (
          <button onClick={() => setSelected([])} className="text-xs text-gray-400 hover:text-gray-700 px-2">
            Clear
          </button>
        )}
      </div>
