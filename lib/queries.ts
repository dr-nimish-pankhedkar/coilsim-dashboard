...
export async function getAllTasks(): Promise<SimulationTask[]> {
  const res = await pool.query<SimulationTask>(`
    SELECT st.id, st.status, st.task_type, st.created_at, st.completed_at,
    ...
    LEFT JOIN cs_py_int.design_cases dc
      ...
      OR st.project_name = dc.project_name
    ORDER BY st.id DESC
  `)
  return res.rows
}
...
export interface RunParams {
  ...
  profileshape_i?: string     // raw content of profileshape.i → written to burnerflux.da
    ...
    const res = await client.query<{ id: number }>(
      `INSERT INTO cs_py_int.simulation_tasks
         (status, task_type, coil_id, feed_id, cot_input, flow_input, project_name,
          ...
          heat_flux_input_type, flux_profile, custom_flux_points, profileshape_i,
